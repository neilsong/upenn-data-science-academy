...
pacman::p_load(tidyverse, dplyr, ggplot2, data.table, lubridate,
               plotROC, usmap, glmnet, car, rworldmap, countrycode, bit64, devtools, lme4, randomForest, xgboost, caret, dummies,xg, DiagrammeR)
```
...
lincc <- df %>%
    group_by(country) %>%
    summarise(model = lm(ccval ~ covid, data=df)) %>%
    select(model)
lininfl <- df %>%
    group_by(country) %>%
