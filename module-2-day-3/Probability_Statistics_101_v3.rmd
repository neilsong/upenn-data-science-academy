...
pacman::p_load(ggplot2, dplyr, scales, latex2exp)
```
...
ggplot(cum_mean_df, 
  ...
  ggtitle("Average winning percentage") +
...
---
\frametitle{95\% $t$-Confidence interval}
...
```{r t and z, echo=F}
# t distribution with df large is approx. same as z
# we see via simulations here
# use base R to pile the two histgrams togehter
par(mfrow=c(1, 2))
hist(rnorm(n=10000), breaks = 100, 
     main = "z scores",
     xlab = "")  # z scores
hist(rt(n=10000, df=199 ), breaks = 100, 
     main = "t scores",
     xlab = "") #t with df= 
```
<!-- \footnotesize -->
