...
pacman::p_load(ISLR, readxl, tidyverse, magrittr, dplyr, ggplot2, ggthemes)
```
...
```{r}
survey_rename$gender
#nothing to be cleaned here right?
```
...
survey_rename %>%
  ggplot(aes(, y=income, fill=gender)) +
...
### This is the graph comparing gender.The blue graph refers to the data set provided, while the black graph is the total female and male in US in 2013
```{r gender_comparison}
...
library(ggplot2)
p<-ggplot(data=df, aes(x=Gender, y=Frequency)) +
...
library(ggplot2)
library(reshape2)
df <- data.frame(education_income_rename)
df <- melt(df, id.vars = "high_school_or_less", variable.name = 'education' )

# Everything on the same plot
ggplot(df, aes(Xax,value, col=variable)) + 
  geom_point() + 
  stat_smooth() 
