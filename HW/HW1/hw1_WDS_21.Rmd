...
### the most commone error was "" which we fixed with survey_rename$"the variable" =="" <- NA
...
#Us <- data.frame(Age =c(UsAge %>% select(Age)),
 #               Pop =c(UsAge %>% select(Number)))
b <- UsAge %>%
  ggplot(aes(y = Age)) +
  geom_bar(stat = "count", color = "dark green") +
  labs(y = "Age Ranges", x = "Amount of People")
b
```
