...
+ In this lecture we introduce LASSO (Least Absolute Shrinkage and Selection Operator) to produce a sparse model. One may view LASSO as a model selection scheme.  `K-Fold` Cross Validation Errors will be introduced and used. 
...
**Remark:** Regarding mean crime rate by state, merely taking mean in the following chunk is not correct due to different county level population sizes. Create **proper average crime rate per 100k** for each state first. The rest follows. 
...
---
\frametitle{Heatmap: then `plot_usmap()`}
